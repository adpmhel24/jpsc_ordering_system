...
from my_app.core.modules.MasterData.branch.schemas import BranchRead
...
from my_app.core.modules.Inventory.inv_adjustment_out.models import (
    InvAdjustmentOutHeader,
    InvAdjustmentOutRow,
)
...
class CRUDAdjustmentOut(
    ...
    def create(
        ...
        *,
        ...
        try:
            ...
            db_header_obj: InvAdjustmentOutHeaderCreate = self.model(
                **header_schema.dict()
            )
            db_header_obj.created_by = user_id
            ...
            for row in rows_schema:
                ...
                db_row_obj = InvAdjustmentOutRow(**row.dict())
                db_row_obj.inv_qty = (
                    row.quantity * alt_uom_group.base_qty * alt_uom_group.alt_qty
                )
                db_header_obj.rows.append(db_row_obj)
            ...
            if not (branch_obj.series_code):
                raise Exception("No series code assigned to this branch!")
            series_num = f"{db_header_obj.id:07d}"
            db_header_obj.reference = f"INV_ADJ_OUT-{branch_obj.series_code.upper()}-{date.today().year}-{series_num}"
    ...
    def cancel(
        self, *, id: int, schema: InvAdjustmentOutHeaderCancel, user_id: int
    ) -> Any:
        db_obj: InvAdjustmentOutHeader = self.get(fk=id)
        if not db_obj:
...
crud_adj_out = CRUDAdjustmentOut(InvAdjustmentOutHeader)
