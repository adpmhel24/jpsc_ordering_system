...
class FormRowModal extends StatelessWidget {
  const FormRowModal({
    ...
    this.invAdjOutItemRow,
  }) : super(key: key);

  final InvAdjustmentOutFormBloc invAdjOutBloc;
  final InventoryAdjustmentOutRow? invAdjOutItemRow;
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: BodyFormRowModal(
        ...
        invAdjOutItemRow: invAdjOutItemRow,
      ),
    );
  }
}
...
class _BodyFormRowModalState extends State<BodyFormRowModal> {
  ...
  void initializeData() async {
    if (widget.invAdjOutItemRow != null) {
      UomGroupModel? oumGroup = await context
          .read<ProductRepo>()
          .getUomGroupAssigned(widget.invAdjOutItemRow!.itemCode);
      _onItemCodeFieldChanged(widget.invAdjOutItemRow!.itemCode);
      _onItemDescFieldChanged(widget.invAdjOutItemRow?.itemDescription ?? "");
      _onQuantityFieldChanged(widget.invAdjOutItemRow!.quantity.toString());
      _onWhseFieldChanged(widget.invAdjOutItemRow!.whsecode);

      if (oumGroup != null) {
        _handleUpdateAltUoms(oumGroup.altUoms!);
      }
      _onUomChanged(widget.invAdjOutItemRow!.uom);
    }
  }
      ...
      onTap: () {
        m.showDialog(
          ...
          builder: (_) => WarehouseSelectionModal(
            ...
            selectedBranch: widget.invAdjOutBloc.state.branch.value,
            onChanged: (value) {
    ...
    return SizedBox(
      ...
      child: CustomFilledButton(
        ...
        onPressed: state.status.isValidated
            ? () {
                InventoryAdjustmentOutRow data = InventoryAdjustmentOutRow(
                ...
                if (widget.invAdjOutItemRow != null) {
                  widget.invAdjOutBloc.add(
                    UpdateRowItem(
                      oldItem: widget.invAdjOutItemRow!,
                      newItem: data,
                    ),
                  );
                } else {
                  widget.invAdjOutBloc.add(AddRowItem(data));
                }
                context.router.pop();
              }
            : null,
