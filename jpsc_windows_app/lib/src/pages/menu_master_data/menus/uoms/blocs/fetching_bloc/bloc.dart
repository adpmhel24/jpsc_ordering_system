import 'dart:io';

import 'package:equatable/equatable.dart';

import 'package:flutter_bloc/flutter_bloc.dart';

import '../../../../../../data/models/models.dart';
import '../../../../../../data/repositories/repos.dart';
import '../../../../../../utils/fetching_status.dart';

part 'event.dart';
part 'state.dart';

class FetchingUoMsBloc extends Bloc<FetchingUoMsEvent, FetchingUoMsState> {
  final UomRepo uomRepo;
  final CurrentUserRepo currUserRepo;
  final ObjectTypeRepo objectTypeRepo;
  FetchingUoMsBloc({
    required this.uomRepo,
    required this.currUserRepo,
    required this.objectTypeRepo,
  }) : super(const FetchingUoMsState()) {
    on<LoadUoms>(_onLoadUoms);
    on<SearchUomsByKeyword>(_onSearchUomsByKeyword);
  }

  void _onLoadUoms(LoadUoms event, Emitter<FetchingUoMsState> emit) async {
    emit(state.copyWith(status: FetchingStatus.loading));
    try {
      // Check if authorized
      int objType = await objectTypeRepo.getObjectTypeByName("Unit Of Measure");
      bool isAuthorized = currUserRepo.checkIfUserAuthorized(
        objtype: objType,
        auths: {"full": true, "create": true},
      );
      if (!isAuthorized) {
        emit(
          state.copyWith(
            status: FetchingStatus.unauthorized,
            errorMessage: "Unauthorized user.",
          ),
        );
      } else {
        await uomRepo.getAll();
        emit(state.copyWith(
            status: FetchingStatus.success, uoms: uomRepo.datas));
      }
    } on HttpException catch (err) {
      emit(state.copyWith(
          status: FetchingStatus.error, errorMessage: err.message));
    } on Exception catch (e) {
      emit(
        state.copyWith(
          status: FetchingStatus.error,
          errorMessage: e.toString(),
        ),
      );
    }
  }

  void _onSearchUomsByKeyword(
      SearchUomsByKeyword event, Emitter<FetchingUoMsState> emit) async {
    emit(state.copyWith(status: FetchingStatus.loading));
    try {
      // Check if authorized
      int objType = await objectTypeRepo.getObjectTypeByName("Unit Of Measure");
      bool isAuthorized = currUserRepo.checkIfUserAuthorized(
        objtype: objType,
        auths: {"full": true, "create": true},
      );
      if (!isAuthorized) {
        emit(
          state.copyWith(
            status: FetchingStatus.unauthorized,
            errorMessage: "Unauthorized user.",
          ),
        );
      } else {
        final data = await uomRepo.offlineSearch(event.value);
        emit(state.copyWith(status: FetchingStatus.success, uoms: data));
      }
    } on HttpException catch (err) {
      emit(state.copyWith(
          status: FetchingStatus.error, errorMessage: err.message));
    } on Exception catch (e) {
      emit(
        state.copyWith(
          status: FetchingStatus.error,
          errorMessage: e.toString(),
        ),
      );
    }
  }
}
