part of 'pq_details_table.dart';

class DataSource extends DataGridSource {
  BuildContext cntx;
  List<PriceQuotationRowModel> itemRows;
  late List<PriceQuotationRowModel> paginatedDatas;
  int startIndex;
  int endIndex;
  int rowsPerPage;
  ...
  void buildPaginatedDataGridRows() {
    dataGridRows = paginatedDatas.asMap().entries.map((element) {
      ...
      PriceQuotationRowModel val = element.value;
  ...
  bool onCellBeginEdit(DataGridRow dataGridRow, RowColumnIndex rowColumnIndex,
      GridColumn column) {
    PriceQuotationModel selectedPriceQuotation =
        cntx.read<PriceQuotationUpdateBloc>().selectedPriceQuotation;
    if (column.columnName == 'Actual Price' &&
        selectedPriceQuotation.pqStatus == 0 &&
        selectedPriceQuotation.docstatus == "O") {
      return true;
    }
    return false;
  }
  ...
  void onCellSubmit(DataGridRow dataGridRow, RowColumnIndex rowColumnIndex,
    ...
    if (column.columnName == 'Actual Price') {
      ...
      notifyListeners();
      // Update the PriceQuotationRows
      cntx
          .read<PriceQuotationUpdateBloc>()
          .add(PriceQuotationRowsChanged(itemRows));
    }
  }
...
class TableSettings {
  ...
  static DataGridRow dataGrid(
      BuildContext context, int index, PriceQuotationRowModel data) {
    return DataGridRow(
      cells: [
        DataGridCell<String>(
